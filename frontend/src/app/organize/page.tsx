...
import { ShieldCheck, Layers, Lock, MousePointer2 } from "lucide-react";
...
export default function OrganizePage() {
  ...
  const addFiles = async (files: File[]) => {
    ...
    try {
      ...
      const sessionId = `organize-${Date.now()}-${Math.random()}`;
      ...
      for (const entry of saved) {
        ...
        sessionStorage.setItem(`pdf-organize-${entry.id}`, JSON.stringify(fileMetadata));
      }
      ...
      // Navigate to editor
      router.push(`/organize/editor?session=${encodeURIComponent(sessionId)}`);
    } catch (error) {
  ...
  return (
    <main className="min-h-screen bg-[#FDFDFF] dark:bg-slate-900 relative py-16 px-4 sm:px-6 transition-colors duration-200">
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none z-0">
        ...
        <div className="absolute top-[20%] -right-[5%] w-[30%] h-[30%] bg-blue-100/40 dark:bg-blue-900/20 rounded-full blur-[100px]" />
      </div>
      ...
      <div className="max-w-3xl mx-auto relative z-10">
        ...
        <header className="text-center mb-12">
          ...
          <h1 className="text-5xl font-black text-slate-900 dark:text-slate-100 tracking-tight mb-4">
            Organize <span className="text-blue-600 dark:text-blue-400">PDF</span>
          </h1>
          <p className="text-slate-500 dark:text-slate-400 text-lg font-medium leading-relaxed max-w-xl mx-auto">
            Atur dan susun ulang halaman PDF sesuai keinginan Anda. 
        ...
        <div className="mt-16 grid grid-cols-1 sm:grid-cols-3 gap-8">
          ...
          <div className="flex flex-col items-center text-center group">
            ...
            <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200 mb-1">Atur Halaman</h4>
            <p className="text-xs text-slate-400 dark:text-slate-500 leading-relaxed">Susun ulang halaman dengan drag & drop untuk urutan yang sempurna.</p>
          </div>
          ...
          <div className="flex flex-col items-center text-center group">
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500 leading-relaxed">Drag, drop, dan atur urutan halaman sesuai keinginan Anda.</p>
          </div>
        </div>
